...
import {ServiceInterface} from "../src/api/servicesManager/interfaces/ServiceInterface";

export class OldFileManipulator {
	...
	public async maybeReplaceCodeBlockMetadata(
		...
		if (newContent !== this._fileContent){
			await this._app.vault.modify(this.file, newContent);
	...
	public async maybeWrite(
		...
		return this._app.vault.modify(this.file, newContent)
			.then(() => {
	...
	public async read(
		...
		const cache: CachedMetadata|null = await this._app.metadataCache.getFileCache(this.file);
		if (cache === null) return false;

		this.cachedFile = cache;

		if (this._fileContent === undefined) this._fileContent = await this._app.vault.read(this.file);
