...
import {StoryCircleStage} from "../../src/services/plotsServices/enums/StoryCircleStage";
...
export class SceneHeaderSubModel extends AbstractHeaderSubModel {
	...
	public async generateData(
		...
		if (this.data.trigger == null || this.data.trigger === '')
			trigger = '<span class="rpgm-missing">SceneModel trigger missing</span>';
		...
		if (this.data.action == null || this.data.action === '')
			action = '<span class="rpgm-missing">SceneModel action missing</span>';
		...
		if (this.settings.useSceneAnalyser) {
			...
			if (this.data.isCurrentlyRunning || this.data.currentDuration > 0)
				response.addElement(new ResponseHeaderElement(this.app, this.currentComponent, 'SceneModel Duration', '', HeaderResponseType.SceneRunTime));
		}
