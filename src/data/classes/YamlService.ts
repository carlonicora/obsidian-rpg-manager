...
export class YamlService {
	public stringify(yaml: any): string {
		let response = "";
	...
	private _stringify(yaml: any | any[], indent: number, isArray = false): string[] {
				...
				if (value == null) {
					...
					switch (typeof value) {
						...
						case "boolean":
							...
							break;
						default:
							response.push(yamlKey + '"' + (value as string).replaceAll("\n", "\\n").replaceAll('"', '\\"') + '"');
							break;
					}
				}
			});
	...
	private _dataType(values: any[]): YamlDataType {
		for (let index = 0; index < values.length; index++) {
			if (typeof values[index] === "object") {
				if (Array.isArray(values[index])) return YamlDataType.Array;
