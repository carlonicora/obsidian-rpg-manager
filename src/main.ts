...
export default class RpgManager extends Plugin implements RpgManagerInterface{
	...
	async onload() {
		...
		/*
		TODO add back
		this.registerView(StaticViewType.Errors.toString(), (leaf) => new ErrorView(this.app, leaf));
		this.registerView(StaticViewType.ReleaseNote.toString(), (leaf) => new ReleaseNoteView(this.app, leaf));
		this.registerView(StaticViewType.RPGManager.toString(), (leaf) => new RPGManagerView(this.app, leaf));
		this.registerView(StaticViewType.Timeline.toString(), (leaf) => new TimelineView(this.app, leaf));

		this.addRibbonIcon('d20', 'RPG Manager', () => {
			this.factories.staticViews.showObsidianView(
				StaticViewType.RPGManager,
			);
		});
		*/
		...
		DatabaseInitialiser.initialise(this.api)
			.then((database: DatabaseInterface) => {
				...
				if (this._isVersionUpdated) {
					//TODO PUT BACK
					// this.factories.staticViews.showObsidianView(StaticViewType.ReleaseNote);
				} else {
					this.app.workspace.detachLeavesOfType(StaticViewType.ReleaseNote.toString());
				}
