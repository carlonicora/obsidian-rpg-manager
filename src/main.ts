...
/*
export const VIEW_TYPE_EXAMPLE = "rpg-manager";
...
class CreationView extends View {
	...
	protected async onOpen(
		...
		console.error(this.database);

		this.icon = 'd20';

		const {containerEl} = this;
		containerEl.empty();
		containerEl.createEl('h2', {text: 'RPG Manager!'});
		this.contentEl = containerEl.createDiv({cls: 'rpgm-view'})

		containerEl.createEl('h3', {text: 'Database status'});
		const status = containerEl.createDiv({cls: 'rpgm-view-status'});
		status.appendText('The RPG Manager Database contains ' + this.database.elements.length + ' elements');
...
export default class RpgManager extends Plugin {
	...
	async onload() {
		...
		/*
		this.addRibbonIcon('d20', "Print leaf types", () => {
			const leaf = this.app.workspace.getRightLeaf(false);
			leaf.open(new CreationView(leaf)).then((view: View) => {
				view.icon = 'd20';
