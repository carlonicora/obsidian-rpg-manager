...
export default function NewChildComponent({
	...
	async function handleCreateNewChild(): Promise<void> {
		...
		const children: ElementInterface[] = api.get(undefined, parent.campaign, type, parent) as ElementInterface[];

		const positionInParent: number =
			children.length === 0
				? 1
				: ((Math.max(...children.map((element: ElementInterface) => element.positionInParent)) + 1) as number);
	...
	return (
		<div className="flex w-full border-b border-b-[--background-modifier-border] pb-1 mb-1 pt-1">
			...
			<div className="flex justify-end w-full text-sm">
				<button className="rpgm-secondary pl-3 pr-3 mr-3" onClick={handleFileAdded}>
					{t("buttons.cancel")}
				</button>
				<button className="rpgm-primary pl-3 pr-3" onClick={handleCreateNewChild}>
