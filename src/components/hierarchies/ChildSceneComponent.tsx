...
export default function ChildSceneComponent({
	...
	if (showTooltip) {
		...
		duration = sceneAnalyser.getExpectedDuration(element.campaign, element.attribute(AttributeType.SceneType)?.value);
	}
		...
		<div className="flex items-center w-full hover:bg-[--background-primary-alt] border-b border-b-[--background-modifier-border] text-sm group">
			...
			{isInPopover ? (
				...
				<div className="grid grid-cols-12 w-full pt-1 pb-1 items-start">
					...
					<div className="pr-1">
						<select
							defaultValue={element.attribute(AttributeType.StoryCircleStage)?.value}
							onChange={(e) => saveAttribute(AttributeType.StoryCircleStage, e.target.value)}
					...
					<div className="col-span-2 pr-1">
						<select
							...
							onChange={(e) => saveAttribute(AttributeType.SceneType, e.target.value)}
							className="selectBorder w-full 
								!border !border-transparent group-hover:!border-[--background-modifier-border] focus:!border-[--background-modifier-border]
								h-7 pl-1 
								focus:!shadow-none !shadow-none
						...
						>
							...
							{Object.entries(SceneType)
								.filter(([key]) => isNaN(Number(key)))
								.map(([key, index]) => {
									const isActive: boolean = isSceneActive(SceneType[key as keyof typeof SceneType]);
									return (
										<option key={key} value={index}>
					...
					<div className="flex justify-center">
						<div>
							<input
								type="checkbox"
								className=""
								defaultChecked={element.attribute(AttributeType.ExternalActions)?.value === true}
								onChange={(e) => saveAttribute(AttributeType.ExternalActions, e.target.checked)}
							/>
						</div>
					</div>
					<div
