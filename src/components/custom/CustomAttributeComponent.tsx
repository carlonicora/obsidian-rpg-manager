...
export default function CustomAttributeComponent({
  ...
  const [type, setType] = React.useState<AttributeComponentType | undefined>(
    attribute?.type,
  );
  ...
  const [canBeDeleted] = React.useState(() => {
    ...
    return !(api.get({}) as ElementInterface[]).some(
      (element: ElementInterface) =>
        element.attribute(attribute.id)?.isSet === true,
    );
  });
  ...
  const validate = (): boolean => {
    ...
    if (options.some((option) => option.includes('"') || option.includes("\\")))
      newErrors.options =
        "Double quotes or back slashes are not allowed in options.";
  ...
  return (
    <div className="w-full">
      ...
      <div
        className={`grid ${
          type === AttributeComponentType.Select ? "grid-cols-3" : "grid-cols-2"
        } gap-4`}
      >
        <div>
          <div>
            ...
            <div>
              ...
              />
              {errors.name && (
                <div className="text-[--text-error] text-sm">{errors.name}</div>
              )}
            </div>
          </div>
          <div>
            ...
            <div>
              <select
                defaultValue={type}
                onChange={(e) =>
                  setType(e.target.value as AttributeComponentType)
                }
              >
                <option></option>
                <option value={AttributeComponentType.Text}>
                  {t("customattributes.text")}
                </option>
                <option value={AttributeComponentType.Number}>
                  {t("customattributes.number")}
                </option>
                <option value={AttributeComponentType.Select}>
                  {t("customattributes.option")}
                </option>
                <option value={AttributeComponentType.Boolean}>
                  {t("customattributes.checkbox")}
                </option>
                <option value={AttributeComponentType.LongText}>
                  {t("customattributes.longtext")}
                </option>
                <option value={AttributeComponentType.Date}>
                  {t("customattributes.date")}
                </option>
                <option value={AttributeComponentType.Link}>
                  {t("customattributes.link")}
                </option>
              </select>
              {errors.type && (
                <div className="text-[--text-error] text-sm">{errors.type}</div>
              )}
            </div>
          </div>
        </div>
            ...
            {Object.values(ElementType).map((type) => (
              <label key={type} className="block">
                <input
                  ...
                  onChange={(e) =>
                    handleCustomTypeChange(type, e.target.checked)
                  }
                />
