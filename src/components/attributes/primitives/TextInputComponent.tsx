...
export default function TextInputComponent({
  ...
  const replaceSequenceWithModalValue = (replacementString: string) => {
    if (lastDetectedPositionRef.current !== null && inputTextRef.current) {
      ...
      const nextChar =
        inputTextRef.current.value.charAt(startPos + 2) || "END_OF_STRING";
      ...
      setTimeout(() => {
        if (inputTextRef.current && shadowSpanRef.current) {
          ...
          shadowSpanRef.current.textContent =
            inputTextRef.current.value.substring(0, newCursorPos);

          const cursorPixelPos = shadowSpanRef.current.offsetWidth;

          inputTextRef.current.scrollLeft =
            cursorPixelPos - inputTextRef.current.offsetWidth / 2;
        }
      }, 0);
    ...
    if (
      ...
      if (event.key === "b") {
        ...
        inputTextRef.current.value =
          inputTextRef.current.value.substring(
            0,
