...
export default function MapAttributeComponent({
  ...
  const [latitude, longitude] = attribute?.value
    ? attribute.value.split(",")
    : [0, 0];

  const allLocations: ElementInterface[] = (
    api.get({ campaign: element.campaign }) as ElementInterface[]
  ).filter(
    (otherElement: ElementInterface) =>
      otherElement.attribute(AttributeType.Location)?.value !== undefined &&
  ...
  function saveAttribute(): void {
    ...
    codeblockService
      .updateCodeblockData(attribute.id, attributeValue)
      .then(() => {
        setEditing(false);
      });
  }
      ...
      <ZoomableGroup center={[longitude, latitude]} zoom={2}>
        ...
        {allLocations.map((location: ElementInterface) => {
          const [latitude, longitude] = location
            .attribute(AttributeType.Location)
            ?.value?.split(",") ?? [0, 0];
          return (
            <Marker
              key={location.id}
