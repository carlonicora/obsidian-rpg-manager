...
export default function ParentAttributeComponent({
  ...
  if (possibleParents === undefined || possibleParents.length === 0)
    return null;
  ...
  const updateParent = (newId: string) => {
    const newParentChildren: ElementInterface[] = api.get({
      campaign: element.campaign,
      type: element.parent.type,
      parent: possibleParents.find(
        (parentElement: ElementInterface) => parentElement.id === newId,
      ),
    }) as ElementInterface[];
  ...
  return (
    <>
      <div className="!font-bold">
        {t("parents.parent", { context: element.parent.type })}
      </div>
      <div>
        {isEditable && (
          <select
