...
export default function AbtStageAttributeComponent({
	element,
	attribute,
	isEditable,
}: {
	...
	if (isEditable) {
		content = (
			<div className="grid grid-cols-1 lg:grid-cols-2 group">
				...
				<div className="flex justify-end !ml-3">
					<button className="rpgm-danger opacity-0 group-hover:opacity-100" onClick={handleDelete}>
						{t("buttons.delete")}
					</button>
				</div>
			</div>
		...
		content = (
			<div>{Object.keys(AbtStage).find((key) => AbtStage[key as keyof typeof AbtStage] === attribute.value)}</div>
		);
	}
