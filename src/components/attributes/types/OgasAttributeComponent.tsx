...
export default function OgasAttributeComponent({ element }: { element: ElementInterface }): React.ReactElement {
	const { t } = useTranslation();
	if (element.type !== ElementType.NonPlayerCharacter) return null;

	const occupation: string | undefined = element.attribute(AttributeType.Occupation)?.value;
	const stake: number | undefined = element.attribute(AttributeType.Stake)?.value;
	const goal: string | undefined = element.attribute(AttributeType.Want)?.value;
	const attitude: string | undefined = element.attribute(AttributeType.Behaviour)?.value;
