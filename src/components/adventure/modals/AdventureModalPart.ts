...
export class AdventureModalPart extends AbstractModalPart {
			...
			if (this._adventures.length === 0){
				...
				mainContent.createEl('h2', {cls: 'rpgm-modal-title', text:
						i18next.t("creation_missing_parent", {
							parent: i18next.t("adventures", {count: 2}),
						}) ?? ''
				});
				mainContent.createSpan({cls: '', text:
						i18next.t("creation_missing_parent", {
							type: i18next.t("adventure", {count: 1}),
							parent: i18next.t("campaign", {count: 1}),
							child: i18next.t("act", {count: 1}),
						}) ?? ''
				});
			} else {
	...
	private _selectAdventureElements(
		...
		groupElement.createDiv({cls: 'rpg-manager-modal-grid-navigation-group-title', text: i18next.t("adventure", {count: 1}) ?? ''});
