...
export default function BrowseImagesComponent({
  ...
  if (
    ...
    allImages = allImages.filter((file: TAbstractFile) =>
      file.path.startsWith(api.settings.assetsFolder ?? "Assets"),
    );
  }
  ...
  if (searchTerm !== "") {
    allImages = allImages.filter((file: TFile) =>
      ...
      <div className="grid grid-cols-4 justify-center gap-3">
        {allImages.map((imageFile: TAbstractFile, index: number) => {
          ...
          if (
            ...
            path = imageFile.path.substring(
              (api.settings.assetsFolder ?? "Assets").length + 1,
            );
          } else {
            path = imageFile.path;
          }
          ...
          return (
            <div
              ...
              onClick={() => handleAddExistingImage(image)}
            >
