...
import {RelationshipList} from "../../services/relationships/RelationshipList";
...
import {FileManipulatorInterface} from "../../services/manipulators/interfaces/FileManipulatorInterface";
...
export abstract class AbstractComponent extends AbstractComponentData implements ComponentModelInterface {
			...
			.forEach((relationship: RelationshipInterface) => {
				if (relationship.component === undefined) {
					...
					if (relationship.type !== RelationshipType.Undefined){
						...
						const maybeRelatedComponents = (database ?? this.database).read<ComponentModelInterface>((component: ComponentModelInterface) =>
