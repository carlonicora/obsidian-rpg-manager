...
Each option should be qualitative, not a short sentence and will allow the storyteller to use it as source of multiple ideas.`;
  ...
  async sendMessage(
    ...
    // );
    try {
      const endpoint = this._api.settings.ollamaUrl
        ? this._api.settings.ollamaUrl + "/v1/chat/completions"
        : this._endpoint;
      const model = this._api.settings.ollamaUrl
        ? this._api.settings.model
        : this._model;
