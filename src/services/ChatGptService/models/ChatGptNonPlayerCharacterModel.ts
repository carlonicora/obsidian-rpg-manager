...
export class ChatGptNonPlayerCharacterModel {
  ...
  private _dataMessages: Map<string, ChatGptMessageInterface> = new Map<
    string,
    ChatGptMessageInterface
  >();
  ...
  constructor(
    ...
    private _model: string = "gpt-4.1-nano",
  ) {
    this._service = new ChatGptService(_api, _model);
  }

  private _generateMessages(
    length: "short" | "long",
  ): ChatGptMessageInterface[] {
    ...
    response.push(
      new ChatGptMessage("system", this._service.context(this._campaign)),
    );
    response.push(new ChatGptMessage("system", this.context()));

    const descriptionMessage: ChatGptMessageInterface | undefined =
      this._dataMessages.get("description");
    if (descriptionMessage) response.push(descriptionMessage);

    const characterArcMessage: ChatGptMessageInterface | undefined =
      this._dataMessages.get("characterArc");
    if (characterArcMessage) response.push(characterArcMessage);

    const beliefsMessage: ChatGptMessageInterface | undefined =
      this._dataMessages.get("beliefs");
    if (beliefsMessage) response.push(beliefsMessage);

    const ghostMessage: ChatGptMessageInterface | undefined =
      this._dataMessages.get("ghost");
    if (ghostMessage) response.push(ghostMessage);

    const lieMessage: ChatGptMessageInterface | undefined =
      this._dataMessages.get("lie");
    if (lieMessage) response.push(lieMessage);

    const needMessage: ChatGptMessageInterface | undefined =
      this._dataMessages.get("need");
    if (needMessage) response.push(needMessage);

    const strengthsMessage: ChatGptMessageInterface | undefined =
      this._dataMessages.get("strengths");
    if (strengthsMessage) response.push(strengthsMessage);

    const weaknessesMessage: ChatGptMessageInterface | undefined =
      this._dataMessages.get("weaknesses");
    if (weaknessesMessage) response.push(weaknessesMessage);

    const behaviourMessage: ChatGptMessageInterface | undefined =
      this._dataMessages.get("behaviour");
    if (behaviourMessage) response.push(behaviourMessage);

    const wantMessage: ChatGptMessageInterface | undefined =
      this._dataMessages.get("want");
    if (wantMessage) response.push(wantMessage);

    const oppositionMessage: ChatGptMessageInterface | undefined =
      this._dataMessages.get("opposition");
  ...
  private _generateSuggestions(
    message: ChatGptMessageInterface,
    length: "short" | "long",
  ): Promise<string[]> {
