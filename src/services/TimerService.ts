...
export class TimerService {
	...
	static endTimer(app: App, api: RpgManagerInterface): void {
		...
		const scene: ElementInterface = api.get(TimerService._runningScene) as ElementInterface;
		if (scene === undefined) return;
		const attribute: AttributeInterface | undefined = scene.attribute(AttributeComponentType.Duration);
		if (attribute === undefined) return;

		const duration = Math.round(Date.now() / 1000) - TimerService._runningStart;

		const codeblockService = new RpgManagerCodeblockService(app, api, scene.file);
		codeblockService.updateCodeblockData(attribute.id, (attribute.value ?? 0) + duration);
