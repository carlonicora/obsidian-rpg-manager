...
export class CustomAttribtuesUpdater {
	static async update(app: App, api: RpgManagerInterface) {
		const campaigns: ElementInterface[] = (api.get() as ElementInterface[]).filter(
			(element: ElementInterface) => element.type === ElementType.Campaign
		);
		if (campaigns.length === 0) return;

		const campaignFiles: TFile[] = campaigns.map((campaign: ElementInterface) => campaign.file);

		const customAttributesContainerPromises: Promise<any[]>[] = campaignFiles.map(async (campaignFile: TFile) => {
			const codeblockService = new RpgManagerCodeblockService(app, api, campaignFile);
			const codeblock: any = await codeblockService.readCodeblock();

			if (!codeblock) return undefined;

			if (!codeblock.attributes || codeblock.attributes.length === 0) return undefined;
		...
		const customAttributesContainer = await Promise.all(customAttributesContainerPromises);
