...
export class FileUploadService {
	...
	async uploadFileList(element: ElementInterface, files: FileList): Promise<void> {
		...
		const hasAssetsFolder = this._api.settings.assetsFolder !== undefined && this._api.settings.assetsFolder !== "";
		...
		const copyPromises = Array.from(files).map((file: any, index: number) => {
			...
			const fileName: string = element.name + " " + (availableFiles.length + index + 1) + extension;
			const link: string = folderName + "/" + fileName;
			const linkForImage: string = hasAssetsFolder ? fileName : link;
