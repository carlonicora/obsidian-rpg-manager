...
export default function ArcComponent({
  element,
  attribute,
  isEditable,
}: {
    ...
    content = Object.entries(ArcType)
      ...
      .map(([key]) => {
        ...
        return (
          <div
            key={key}
            className="flex items-center border border-[--background-modifier-border] rounded-md m-4 p-4 hover:bg-[--background-secondary] cursor-pointer"
          ...
          >
            ...
            <div>
              <h4 className="!m-0 !mb-2">
                {t("arc.arc", { context: arcType })}
              </h4>
              <small>
                <MarkdownComponent
                  value={t("arc.description", { context: arcType })}
                />
              </small>
            </div>
          </div>
    ...
    content = (
      ...
      >
        {attribute.value && (
          <>
            <div className="!mb-2">
              {t("arc.arc", { context: attribute.value })}
            </div>
            <small>
              <MarkdownComponent
                value={t("arc.description", { context: attribute.value })}
              />
            </small>
          </>
        )}
